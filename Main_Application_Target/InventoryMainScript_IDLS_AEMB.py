...
class MD_InventoryEXI_SMV_GUI(App):
    ...
    title = "Excel Inventory To Simplified Model View" #Excel to Simplified Model VIew in GUI
    ...
    """
    (Callable Method, Function) => def build(self), The Application Builder that uses KV File
    ...
    """
    def build(self):
    # rand_primary_palette is a copy of theme_cls.primary_palette that has Option Property. It was copied due to random.choice only produce '1'.
        ...
        self.theme_cls.primary_palette = (choice(rand_primary_palette))
        self.theme_cls.primary_hue = '400'
        self.theme_cls.theme_style = 'Light'
        return self.MainClassBuildFile
    """
    (Callable Method, Function) => def on_start(self, no any other arguments passed.)
    ...
    (Callable Method, Function) => def CriticalComponent_Check(self), The Gate to Initialize
        ...
        NOTE: LoggerDebug is part of the Kivy, it was used to print out specific data, just like print but with formatting
    """
        ...
        try:
            ...
            for EachDataSheet in ExcelWorksheet.iter_rows(min_row=int(self.root.ids.DataBind_RowCellStartPoint.text), max_row=int(self.root.ids.DataBind_RowCellEndPoint.text), min_col=column_index_from_string(self.root.ids.DataBind_ColumnCellStartPoint.text), max_col=column_index_from_string(self.root.ids.DataBind_ColumnCellEndPoint.text)):
                for cell in EachDataSheet:
                    ...
                    print('[Excel Data - Generate Report] Content Element Loop #', Content_ListElement, 'Cell Stats', cell,'| Cell Value', cell.value)
                    if Content_ListElement == 1 and Content_ListElement <= 15:
                        if cell.value is None or cell.value == 'None' or cell.value == '' or cell.value == '-':
                        ...
                        if cell.value is None or cell.value == '' or cell.value == '-':
                            ...
                            self.DataReport_Calculated_TotalCost += cell.value
            ...
            self.root.ids.DataReport_TotalProposed.text = str(self.DataReport_CalculatedProposed)
            ...
            self.MDUserNotif_SnackbarHandler(str(ErrorMessage))
    """
    (Callable Method, Function) => def MDButton_Exit_SaveData(self), called by on_stop to save data of last strings on SQL
        This function saves important textfields to SQL Database inorder to to retrieve data on next load of the program.
    """
    ...
    def ExcelLoadInit_Data_Editor(self, ExcelLoad_Method):
        ...
        try:
            ...
            if ExcelLoad_Method == 'Unload' or ExcelLoad_Method == 'Reload':
                ...
                self.MDUserNotif_SnackbarHandler("Data has been removed from the Application.")
                """
            ...
            self.MDUserNotif_SnackbarHandler("File Data Load Failed... Check the files or the arguments you set on the Data Import Data / Export Pull Out Section!")
            self.MDUserNotif_SnackbarHandler(str(ErrorMessage))
    """
    ...
    def MDWorksheetWorker_DataEditor_ValueRaiseLowCaller(self, Button_IDNumber, TypeAction_ValueChange):
        Placement_Val_Conversion_DataHold = 0
        try:
            if Button_IDNumber == 1:
                if TypeAction_ValueChange == 'Increment':
                    try:
                        ...
                        self.MDUserNotif_SnackbarHandler('Im sorry, I cannot increment this value. (Was this value a float or string?)')
                elif TypeAction_ValueChange == 'Decrement':
                    try:
                        self.root.ids.Resource_ItemNumVal.text = str(int(self.root.ids.Resource_ItemNumVal.text) - 1)
                    except:
                        self.MDUserNotif_SnackbarHandler('Im sorry, I cannot decrement this value. (Was this value a float or string?)')
                else:
